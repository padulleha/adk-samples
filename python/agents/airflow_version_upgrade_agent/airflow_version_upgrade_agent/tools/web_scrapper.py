...
def get_urls_from_vertex(query: str, project_id: str, location: str, app_id: str) -> list[str]:
...
def scrape_urls_content(urls: list[str]) -> str:
    ...
    for url in urls:
        try:
            ...
            if not soup.body:
                logging.warning(f"Could not find <body> tag in {url}. Skipping.")
                continue
...
def research_operator_documentation(operators: list[str], source_version: str, target_version: str, project_id: str) -> list[dict]:
    results =[]
